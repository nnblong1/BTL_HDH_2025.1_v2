 ...
 * 
 * FIXED ISSUES:
 * - I2C adapter detection now uses device tree properly
 * - Character device creation fixed for modern kernel API
...
static int unicam_probe(struct platform_device *pdev)
{
	...
	/* Find I2C adapter for camera - WITH BETTER ERROR HANDLING */
	...
	if (!adapter) {
		...
		dev_err(&pdev->dev, "Please check:\n");
		dev_err(&pdev->dev, "  1. Camera is connected properly\n");
		dev_err(&pdev->dev, "  2. I2C is enabled in device tree\n");
		dev_err(&pdev->dev, "  3. Run: sudo i2cdetect -l\n");
		dev_err(&pdev->dev, "  4. Run: sudo i2cdetect -y 10 (or 0)\n");
		
		/* DON'T fail probe - allow driver to load without camera */
...
skip_i2c:
	...
	/* Create device class - FIXED for modern kernels */
